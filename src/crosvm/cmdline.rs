...
#[cfg(feature = "vnc")]
use crate::crosvm::config::VncConfig;
#[cfg(feature = "plugin")]
...
/// Parse a VNC server configuration string in key=value format.
#[cfg(feature = "vnc")]
fn vnc_server_from_str(s: &str) -> Result<VncConfig, String> {
    from_key_values(s)
}
...
pub struct RunCommand {
    ...
    #[argh(
        option,
        arg_name = "kind=KIND[,card=NAME][,camera_id=ID][,role=ROLE][,allow_sw=BOOL][,uid=N][,gid=N]"
    )]
    #[serde(default)]
    #[merge(strategy = append)]
